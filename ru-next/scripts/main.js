...
function populateTableEntry(trainee) {
  ...
  <div class="table__entry ${eliminated}">
    ...
    <div class="table__entry-text">
      ...
      <span class="companyandyear">${stringToFlag(trainee.country)} <span>•
      ${trainee.birthyear}</span></span>
    </div>
  </div>`;
  return tableEntry;
}
...
function populateRanking() {
  ...
  for (let i = 0; i < rowNums.length; i++) {
    ...
    for (let j = 0; j < rowNums[i]; j++) {
      ...
      rankRow.insertAdjacentHTML(
        "beforeend",
        populateRankingEntry(currTrainee, currRank)
      );
      ...
      if (currTrainee.id >= 0) {
        ...
        dragIcon.addEventListener(
          "dragstart",
          createDragStartListener(currRank - 1)
        );
      }
...
function stringToFlag(countries) {
  let flags = "";
  const t = countries.split("/");
  t.map((country) => {
    flags += `<img class="flag" src="assets/flags/${country.toLowerCase()}.svg" />`;
  });
  return flags;
}

function populateRankingEntry(trainee, currRank) {
  let modifiedCountry = trainee.country;
  if (modifiedCountry !== "&#8203;")
    modifiedCountry = stringToFlag(modifiedCountry);
  let eliminated = showEliminated && trainee.eliminated && "eliminated";
  let top6 = showTop6 && trainee.top6 && "top6";
  const rankingEntry = `
  <div class="ranking__entry ${eliminated}">
    <div class="ranking__entry-view">
      <div class="ranking__entry-icon">
        <img class="ranking__entry-img" src="assets/trainees/${
          trainee.image
        }" />
        <div class="ranking__entry-icon-border ${trainee.grade.toLowerCase()}-rank-border" data-rankid="${
    currRank - 1
  }"></div>
      </div>
    ...
    <div class="ranking__row-text">
      ...
      <div class="flags">${modifiedCountry}</div>
    </div>
  </div>`;
  return rankingEntry;
}
...
function filterTrainees(event) {
  ...
  // filters trainees based on name, alternate names, and country
  filteredTrainees = trainees.filter(function (trainee) {
    let initialMatch =
      includesIgnCase(trainee.name_romanized, filterText) ||
      includesIgnCase(trainee.country, filterText);
    // if alernates exists then check them as well
    let alternateMatch = false;
    let alternates =
      alternateRomanizations[trainee.name_romanized.toLowerCase()];
    if (alternates) {
      for (let i = 0; i < alternates.length; i++) {
        alternateMatch =
          alternateMatch || includesIgnCase(alternates[i], filterText);
      }
    }
...
function addRankedTrainee(trainee) {
  for (let i = 0; i < ranking.length; i++) {
    if (ranking[i].id === -1) {
      // if spot is blank denoted by -1 id
      ranking[i] = trainee;
      return true;
    }
  }
...
const currentURL = "https://momorin90.github.io/ru-next/";
