...
function display() {
  ...
  const charNameDisp = (name, groups) => {
    let groupStr = "";
    if (groups.length === 1) {
      groupStr = groups[0];
    } else {
      const filteredGroups = groups.filter((n) => {
        return n !== "Solo Artist";
      });
      groupStr = filteredGroups.join(", ");
    }
    //const charName = reduceTextWidth(name, "Arial 12.8px", 220);
    //const charTooltip = name !== charName ? name : "";
    return `<p class="name">${name}</p><p class="group">${groupStr}</p>`;
  };
...
function result(imageNum = 5) {
  ...
  const imgRes = (char, num) => {
    ...
    return `<div class="result image"><div class="left">#${num}</div><img src="${char.img}"><div class="legend"><div class="name">${char.name}</div><div class="group">${char.opts.groups[0]}</div></div></div>`;
  };
  const res = (char, num) => {
    //const charName = reduceTextWidth(char.name, "Arial 12px", 160);
    //const charTooltip = char.name !== charName ? char.name : "";
    //return `<div class="result"><div class="left">${num}</div><div class="right"><span title="${charTooltip}">${charName}</span></div></div>`;
    return `<div class="result card"><div class="left">${num}</div><div class="right"><div class="name">${char.name}</div><div class="group">${char.opts.groups[0]}</div></div></div>`;
  };
  ...
  // Add results style
  resultTable.style.background = "#e6e6fa";
  resultTable.style.padding = "10px";
  resultTable.style.margin = "1em auto";
  ...
  characterDataToSort.forEach((val, idx) => {
    ...
    if (imageDisplay-- > 0) {
      ...
      if (imageDisplay == -1) {
        resultTable.insertAdjacentHTML("beforeend", '<div class="separator"/>');
      }
...
function populateOptions() {
  ...
  const optInsertLarge = (
    ...
    return `<div class="large option"><label class="toplabel" title="${
      tooltip ? tooltip : name
    ...
    <button class="uncheck" ${
      disabled ? "disabled" : ""
    } id="uncheck-${id}"> Uncheck all</button>
    ${
      name === "Filter by Group"
        ? '<div class="warning">It is highly recommended to select only the groups you want to include in the sorter, as a complete selection will result in thousands of matches</div>'
        : ""
    }
    </div>`;
  };
  ...
  options.forEach((opt) => {
    if ("sub" in opt) {
      ...
      const uncheckbtn = document.getElementById(`uncheck-${opt.key}`);
